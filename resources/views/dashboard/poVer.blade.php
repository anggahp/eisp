        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ url('/pover') }}" method="POST" class="form-body" role="form" autocomplete="off" id="frmUploadKanban">
                        @csrf
...
<script>
    var table = '';
    var vUrl = '{{ route('pover') }}';
    var vtampDate = '{{ Request::segment(2) }}';

    if (vtampDate != '') {
        vUrl = '{{ route('pover') }}'+'/'+vtampDate+'/date';
    }
    $(function() {         
        $.ajaxSetup({
            headers: {
...
<script>
    ...
    $('#btnCari').on('click', function() {
        ...
        window.location.href="{{ route('pover') }}"+"/"+strDate+"/date";
    });

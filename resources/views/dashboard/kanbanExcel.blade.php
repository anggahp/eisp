            ...
            <div id="accordion" class="panel-group">
                <div class="card card-default">
                    <div class="card-header" >
                        <h4 class="card-title" style="color: #32395c;"> LIST DATA KANBAN </a>
                        </h4>
                    </div>
                    <div class="card-body">                                
                                ...
                                <table class="table table-hover table-responsive table-bordered mb-0 data-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="txtCenter" width="60">Supp</th>
                                            <th class="txtCenter" width="60">Tgl. DN</th>
                                            <th class="" width="750">Deskripsi</th>
                                            <th class="txtCenter" width="120">Updated</th>
                                            <th class="txtCenter" width="140">Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ url('/kanbanexcel') }}" method="POST" class="form-body" role="form" autocomplete="off" id="frmUploadKanban">
                        @csrf
                        <input type="hidden" id="KANBANID" name="KANBANID"> 
                        <input type="hidden" id="FILENAME_UPD" name="FILENAME_UPD"> 
                        <div class="row">                            
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="SUPPLIERCODE">Supplier Code</label>
                                    {{-- <input type="text" class="form-control" id="SUPPLIERCODE" name="SUPPLIERCODE" placeholder="Supplier Code"  onkeyup="this.value = this.value.toUpperCase();" required> --}}
                                    <select  class="form-control" id="SUPPLIERCODE" name="SUPPLIERCODE" required>
                                    @foreach ($supps as $supp)
                                        <option value="{{$supp->SUPPLIERCODE}}">{{$supp->SUPPLIERCODE.' - '.$supp->SUPPLIERNAME}}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>                            
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="DATADATE">Tanggal DN</label>
                                    <input type="text" class="form-control datepicker" id="DATADATE" name="DATADATE" placeholder="dd/mm/yyyy" readonly required style="background-color: transparent;">
                                </div>
                            </div> 
                        </div>
                        <div class="row"> 
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="DESCRIPTION" name="DESCRIPTION" placeholder="Deskripsi" onkeyup="this.value = this.value.toUpperCase();" required>
                                </div>
                            </div>                             
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <span class="control-fileupload">
                                    <label for="file" class="text-left" id="vFileName">Pilih Foto di Komputer Anda.</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" id="KANBANID_DEL"> 
                    {{-- <br /> --}}
                    <div class="modal-footer">
...
<script>
    var table = '';
    var vUrl = '{{ route('kanbanexcel') }}';
    var vtampDate = '{{ Request::segment(2) }}';

    if (vtampDate != '') {
        vUrl = '{{ route('kanbanexcel') }}'+'/'+vtampDate+'/date';
    }
    $(function() {         
        $.ajaxSetup({
            headers: {
...
<script>
    ...
    $('#btnCari').on('click', function() {
        ...
        window.location.href="{{ route('kanbanexcel') }}"+"/"+strDate+"/date";
    });
...
<script>
    ...
    $(document).on('click', '.add-modal', function() {
        ...
        $('#SUPPLIERCODE').val('');
        $('#DESCRIPTION').val('');
        $('#vFileName').text('Pilih Foto di Komputer Anda.');
    ...
    $(document).on('click', '.edit-modal', function() {
        ...
        $.ajax({
            type: 'GET',
            url : '{{ url('/kanbanexcel') }}/'+id+'/edit',
            data: '',
            success: function(data)
            {
                $('.title-pend').text('Ubah Data Kanban');
                $('#KANBANID').val(data.KANBANID);
                $('#FILENAME_UPD').val(data.FILENAME);
                $('#DATADATE').datepicker('update',new Date(data.DATADATE));
